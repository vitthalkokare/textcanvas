...
const Canvas: React.FC<CanvasProps> = ({
  ...
  const handleCanvasClick = (e: React.MouseEvent) => {
    ...
    if (selectedShapeType) {
      const newShape = { id: shapes.length + 1, type: selectedShapeType, x, y, width: 100, height: 100, color: 'transparent' };
  ...
  const handleAddNode = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter' && inputPosition) {
      const newNode = { id: nodes.length + 1, text: newText, x: inputPosition.x, y: inputPosition.y, children: [] };
  ...
  const handleConnectNode = (id: number) => {
    if (selectedNode) {
      setNodes((prevNodes) =>
        prevNodes.map((n) =>
          n.id === selectedNode.id ? { ...n, children: [...(n.children || []), id] } : n
        )
      );
      setSelectedNode(null);
    } else {
  ...
  return (
    <div
      ref={containerRef}
      className={`flex-1 ${isDarkMode ? 'bg-gray-100' : 'bg-gray-900'} relative overflow-auto`}
    ...
    >
      ...
      {nodes.map((node) =>
        node.children?.map((childId) => {
          ...
          return (
            <svg key={`line-${node.id}-${childId}`} className="absolute w-full h-full" style={{ left: 0, top: 0 }}>
              <line
                x1={node.x + 25}
                y1={node.y + 25}
                x2={childNode ? childNode.x + 25 : childShape!.x + childShape!.width / 2}
                y2={childNode ? childNode.y + 25 : childShape!.y + childShape!.height / 2}
                stroke="black"
                strokeWidth="2"
              />
            </svg>
